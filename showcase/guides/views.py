    ...
    if request.method == "POST":
        ...
        if insertreq.is_valid():
            ...
            return insert_succ(data["company"], data["region"], data["city"], data["product"],
                               data["urlline"], data["ispizza"], data["per_box"])
        else:
            error = "ERROR"
            form = insertreq
    else:
...
def insert_succ(company, region, city, product, url, ispizza, per_box):
    try:
        own_target = "http://127.0.0.1:8000/models/"
        target_class = model_classes[company]
        url = own_target + str(company)+"/"+str(region)+"/"+str(city)+"/"+str(product)+"/"+url
        main_classes = baseclass.objects.filter(region=region, city_ID=city, product_ID=product)
        ...
        attempt = baseclass(region=region, city_ID=city, product_ID=product,
                            target=url, ispizza=ispizza, per_box=per_box)
